...
export default function CollegesPage() {
  ...
  // --- Sync URL changes from Navbar/Homepage to local state ---
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchColleges = async () => {
      ...
      try {
        ...
        // 👇 FIX: Calculate live ratings directly from the fetched data
        const formattedData = result.data.map((college: any) => {
          const reviewsArr = college.reviews || [];
          
          // Get true review count
          const totalReviews = reviewsArr.length > 0 
            ? reviewsArr.length 
            : (college._count?.reviews || 0);
          
          // Get true average rating
          ...
          return {
            ...
            // Apply live math, formatted properly
